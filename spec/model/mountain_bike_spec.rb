...
describe MountainBike do
  ...
  describe 'off_road_ability' do
    ...
    context 'rigid' do
      let(:mountain_bike) { RigidMountainBike.new(instance_params_of_rigid) }
      it { should == 2.5 }
    end
    context 'front_suspension' do
      let(:mountain_bike) { FrontSuspensionMountainBike.new(instance_params_of_front_suspension) }
      it { should == 13 }
    end
    context 'full_suspension' do
      let(:mountain_bike) { FullSuspensionMountainBike.new(instance_params_of_full_suspension) }
      it { should == 38 }
    end
  end
